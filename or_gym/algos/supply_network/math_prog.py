...
def im_pi_lp_model(env,use_expectation=False):
    '''
...
def im_sh_lp_model(env,use_expectation=False):
    '''
    Build a shrinking horizon LP model for the InvManagement problem (v2 and v3).
    The model assumes the average demand from the specified distribution is used
    (unless a user specified demand profile is given).
    ''' 
    ...
    backlog = env.backlog #backlog or lost sales
    #demand profile
    D = {e:np.ones(env.num_periods-env.period)*np.mean(env.graph.edges[e]['demand_dist']) if isinstance(env.graph.edges[e]['demand_dist'],(list, np.ndarray)) else np.ones(env.num_periods-env.period)*env.graph.edges[e]['demand_dist'].mean(**env.graph.edges[e]['dist_param']) for e in lp.retail_links} #demands on a retail link for each period
    lp.D = pe.Param(lp.demands, initialize = {te:D[te[1:]][te[0]] for te in lp.demands}) #store demands
    prob = {e:[1/(env.num_periods-env.period) for t in range(env.num_periods-env.period)] if isinstance(env.graph.edges[e]['demand_dist'],(list, np.ndarray)) else env.graph.edges[e]['demand_dist'].pmf(D[e],**env.graph.edges[e]['dist_param']) for e in lp.retail_links} #probability of each demand based on distribution
    lp.prob = pe.Param(lp.demands, initialize = {te:prob[te[1:]][te[0]] for te in lp.demands}) #store probability at each period
    ...
    if use_expectation:
        ...
        lp.obj = pe.Objective(expr = 1/(env.num_periods-env.period) * sum(lp.P[t,j] for j in lp.J for t in lp.T),
                        sense = pe.maximize)
    
    return lp

def im_rh_lp_model(env, window_size, use_expectation=False):
    '''
    Build a rolling horizon LP model for the InvManagement problem (v2 and v3).
    The model assumes the average demand from the specified distribution is used
    (unless a user specified demand profile is given).
    ''' 
