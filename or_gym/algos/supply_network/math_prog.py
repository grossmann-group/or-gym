...
def im_pi_lp_model(env):
    ...
    #demand profile
    D = {e:env.graph.edges[e]['demand_dist'] if isinstance(env.graph.edges[e]['demand_dist'],list) else env.graph.edges[e]['demand_dist'].rvs(size=env.num_periods,**env.graph.edges[e]['dist_param']) for e in lp.retail_links} #demands on a retail link for each period
    lp.D = pe.Param(lp.demands, initialize = {te:D[te[1:]][te[0]] for te in lp.demands}) #store demands
    prob = {e:[D[e].count(D[e][t])/len(D[e]) for t in range(env.num_periods)] if isinstance(env.graph.edges[e]['demand_dist'],list) else env.graph.edges[e]['demand_dist'].pmf(D[e],**env.graph.edges[e]['dist_param']) for e in lp.retail_links} #probability of each demand based on distribution
...
def im_sh_lp_model(env):
    ...
    lp.reorder_links = pe.Set(dimen=2, initialize=env.reorder_links)
    lp.retail_links = pe.Set(dimen=2, initialize=env.retail_links)
    lp.demands = pe.Set(dimen=3, initialize=[(t,) + e for e in env.retail_links for t in range(env.num_periods-env.period)])
    lp.T = pe.RangeSet(0,env.num_periods-1-env.period) 
    lp.T1 = pe.RangeSet(0,env.num_periods-env.period)
    ...
    backlog = env.backlog #backlog or lost sales
    #demand profile
    D = {e:env.graph.edges[e]['demand_dist'][env.period:] if isinstance(env.graph.edges[e]['demand_dist'],list) else np.ones(env.num_periods-env.period)*env.graph.edges[e]['demand_dist'].mean(**env.graph.edges[e]['dist_param']) for e in lp.retail_links} #demands on a retail link for each period
    lp.D = pe.Param(lp.demands, initialize = {te:D[te[1:]][te[0]] for te in lp.demands}) #store demands
    prob = {e:[D[e].count(D[e][t])/len(D[e]) for t in range(env.num_periods-env.period)] if isinstance(env.graph.edges[e]['demand_dist'],list) else env.graph.edges[e]['demand_dist'].pmf(D[e],**env.graph.edges[e]['dist_param']) for e in lp.retail_links} #probability of each demand based on distribution
    lp.prob = pe.Param(lp.demands, initialize = {te:prob[te[1:]][te[0]] for te in lp.demands}) #store probability at each period
    ...
    #objective function: maximize average profit
    lp.obj = pe.Objective(expr = 1/(env.num_periods-env.period) * sum( prod([lp.prob[t,e[0],e[1]] for e in env.retail_links]) * sum(lp.P[t,j] for j in lp.J) for t in lp.T),
                          sense = pe.maximize)
