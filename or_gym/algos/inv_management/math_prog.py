...
def build_im_dyn_mip_model(env):#,bigm=10000):
    '''
    Build a dynamic policy math programming model for the InvManagement problem.
    ...
    #define sets
    mip.n = pe.RangeSet(0,env.num_periods-1 - env.period) 
    mip.n1 = pe.RangeSet(0,env.num_periods - env.period)
    mip.m = pe.RangeSet(0,env.num_stages-1) #stages
    mip.m0 = pe.RangeSet(0,env.num_stages-2) #stages (excludes last stage which has no inventory)
    ...
    mip.D = pe.Param(mip.n, initialize = {i:D for i in mip.n}) #store demands
    prob = env.demand_dist.pmf(D,**env.dist_param) #probability of each demand based on distribution
    mip.prob = pe.Param(mip.n, initialize = {i:prob for i in mip.n}) #store probability at each period
    ...
    #initialize
    for m in mip.m0:
        mip.I[0,m].fix(env.I[env.period,m])
        mip.T[0,m].fix(env.T[env.period,m])
